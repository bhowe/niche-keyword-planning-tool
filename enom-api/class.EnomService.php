...
/*
 ...
 - 24 April 2006: Added support for command SynchAuthInfo
*/
...
Class EnomService extends EnomInterface
{
  ...
  *
  * Executes the 'SynchAuthInfo' command on Enom's servers, to create a new EPP key for a domain
  * name that belongs to your Enom account, synchronize the key with the Registry, and optionally
  * email it to the registrant.
  *
  * @param      string      $domainName     Must be valid and belongs to your Enom account.
  * @param      bool        $sendEmail      Should mail the EPP key to the registrant? (default is true).
  * @param      bool        $runSynch       Should synchronize the EPP key and make sure Enom's records are identical to the Registry's? (default is true).
  * @return     bool        True if succeed and False if failed.
  * @access     public
  */
  function synchronizeEPP($domainName, $sendEmail = true, $runSynch = true)
  {
    $this->_setCommand('SynchAuthInfo');
    ...
    $this->addParam('EmailEPP', $sendEmail ? '1' : '0');
    $this->addParam('RunSynchAutoInfo', $runSynch ? '1' : '0');
